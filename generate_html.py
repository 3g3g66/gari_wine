...
# Raggruppa e aggiunge le sezioni
for section in df['Category'].dropna().unique():
    wines = df[df['Category'] == section]
    html += f'<section id="{section}">'
    html += f'<h2>{section}</h2>'
    for _, row in wines.iterrows():
        html += f'<div class="entry">${row["Price"]} {row["Country"]} {row["Vintage"]} {row["Name"]} – {row["Producer"]}</div>'
    html += '</section>'
